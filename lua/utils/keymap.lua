...
function keymap.silent(opt)
  return function() opt.silent = true end
end

function keymap.noremap(opt)
  return function() opt.noremap = true end
end

function keymap.expr(opt)
  return function() opt.expr = true end
end

function keymap.remap(opt)
  return function() opt.remap = true end
end

function keymap.nowait(opt)
  return function() opt.nowait = true end
end
...
function keymap.new_opts(...)
  ...
  if #args == 0 then return o.options end
...
--- @param str string
--- @return string
function keymap.cmd(str) return "<cmd>" .. str .. "<CR>" end

-- visual
--- @param str string
--- @return string
function keymap.cu(str) return "<C-u><cmd>" .. str .. "<CR>" end
